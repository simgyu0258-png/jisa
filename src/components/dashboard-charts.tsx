...
type IssueBar = { issue: string; quantity: number };
...
export function DashboardCharts({
  ...
  return (
    <div className="grid grid-cols-1 gap-4">
      ...
      <section className="rounded-lg border border-slate-200 bg-white p-4">
        <h3 className="mb-3 text-sm font-semibold text-slate-800">호별 판매 부수</h3>
        <ResponsiveContainer width="100%" height={288}>
          <BarChart data={issueBars} barCategoryGap="25%">
            <CartesianGrid strokeDasharray="3 3" />
            <XAxis dataKey="issue" />
            <YAxis />
            <Tooltip />
      ...
      <section className="rounded-lg border border-slate-200 bg-white p-4">
        ...
        <ResponsiveContainer width="100%" height={288}>
          <LineChart data={monthlyLine}>
            ...
            <Tooltip />
            <Line dataKey="quantity" type="monotone" stroke="#0f172a" strokeWidth={2} dot={{ r: 3 }} />
          </LineChart>
        </ResponsiveContainer>
      </section>
    </div>
