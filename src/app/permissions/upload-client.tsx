...
type PreviewResponse = {
  ...
  payload: Array<{ branchId: number; permissions: boolean[] }>;
};

export function PermissionsUploadClient() {
  ...
  async function requestPreview(formData: FormData) {
    ...
    setMessage("");
    try {
      const response = await fetch("/api/permissions/excel/preview", {
        method: "POST",
        body: formData,
      });
  ...
  async function applyPreview() {
    ...
    setLoading(true);
    try {
      const response = await fetch("/api/permissions/excel/apply", {
        method: "POST",
  ...
  return (
    <section className="space-y-3 rounded-lg border border-slate-200 bg-white p-4">
      <h2 className="text-lg font-semibold">엑셀 업로드 (권한 일괄 수정)</h2>
      <form
        action={(formData) => {
